...
function indexing(page, keys, values) {
    offset = page * 100;
    let req = client.get("http://repo.mel.cgiar.org/rest/items?expand=metadata,parentCollection&limit=100&offset=" + offset, function (data, response) {
        if (_.isEmpty(data)) {
            if (retry <= 4) {
                console.log('retry => ' + retry++);
                indexing(page, keys, values);
                return;
            } else {
                return;
            }
        ...
        data.forEach(element => {
            ...
            if (formated.dc_date_issued != undefined)
                formated['year'] = formated.dc_date_issued.split("-")[0]
            formated['repo'] = "MELSPACE";
            finaldata.push({ index: { _index: 'publication', _type: 'report', _id: 'mel_' + formated.id } });
